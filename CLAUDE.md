...
- **Creation, distribution, and anchoring are distinct.** A DID is derived locally from its signed creation operation, without a registrar or registry transaction. Hyperswarm gossip distributes non-local operations, including chain-registered operations and batch assets. IPFS is fallback content retrieval, not the source of creation authority or the primary distribution mechanism. Use locally retained evidence first; registry confirmation and ordering remain separate. Local-only DIDs are not queued for gossip.
