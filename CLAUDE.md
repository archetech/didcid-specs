...
- **Agent vs. asset.** Agents hold keys and control their own document; assets may publish keys but are controlled by exactly one agent. Most rules in `creation.md`, `update.md`, and `resolution.md` fork on this distinction, and it constrains other sections — e.g. it is why `recovery.md` has no multi-sig path.
